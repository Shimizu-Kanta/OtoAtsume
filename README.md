...
- `/covers` はカード表示とリスト表示を切り替えできます。
- 表示形式は `?view=card` / `?view=list` の URL クエリで共有できます。
- URL に表示形式がない場合は `coversViewMode` localStorage の値を使い、不正な値の場合はカード表示に戻します。
- YouTube の `watch` / `youtu.be` / `shorts` / `live` / `embed` URL は `img.youtube.com` の `hqdefault.jpg` サムネイルを表示します。
- YouTube API key や Data API は使わず、サムネイル URL は DB に保存しません。

## Performer 拡張

- Performer は任意の `colorCode` と `debutDate` を持てます。
- タグは管理者のみ作成・編集・付与できます。public 側ではタグの表示と絞り込みのみ可能です。
- Performer CSV import は `colorCode`, `debutDate`, `tags` に対応しています。
- CSV の `tags` と `aliases` は `;` 区切りです。

```csv
name,groupName,youtubeUrl,officialUrl,colorCode,debutDate,tags,aliases
星街すいせい,ホロライブ,https://www.youtube.com/@HoshimachiSuisei,,#4A90E2,2018-03-22,"Vsinger;歌枠;オリ曲あり","Hoshimachi Suisei;すいちゃん;Suisei"
```
...
Cloud Run では `https://oto-atsume-564734330008.asia-northeast1.run.app` と `https://oto-atsume-lk5xfod4lq-an.a.run.app` のように、複数形式の URL が見える場合があります。実際に公開・共有する URL は 1 つに決め、以下をすべて同じ公開 URL に揃えてください。

- `NEXT_PUBLIC_SITE_URL`
- `NEXTAUTH_URL`
- Google OAuth の Authorized redirect URI: `https://<公開URLのhostname>/api/auth/callback/google`
- Cloudflare Turnstile の許可 hostname

公開 URL を変更した場合は、Secret Manager の該当 secret を更新し、Cloud Run の新しい revision を deploy してください。URL が混在すると、OAuth callback や CAPTCHA hostname 検証で失敗する原因になります。
...
- `/` が表示できる
- `/covers` が表示できる
- `/covers/new` で CAPTCHA 完了後に登録できる
- `/covers/:id/report` で通報できる
- `/performer-applications/new` で活動者申請できる
- `/admin` に URL 直打ちで入れる
- 管理者ログインできる
- 管理画面で申請・通報を確認できる
- Cloud Run logs に ERROR が出ていない
- Google OAuth redirect URI が公開 URL と一致している
- Cloudflare Turnstile hostname が公開 URL と一致している
- `NEXT_PUBLIC_SITE_URL` と `NEXTAUTH_URL` が公開 URL に揃っている
- Cloud SQL automated backups が有効
- migration 前に backup を取る
